...
import { trendSeries, AGENTS } from "@/lib/omnicow/data";
...
  // Compute clusters and adoption rates from farmers data
...
  const rate = fs.length > 0 ? Math.round((fs.reduce((s, f) => s + f.day90, 0) / fs.length) * 100) : 0;
  return { label: `Cluster ${c}`, rate };
 });

  // byTopic remains mock for now (as per task focus on FARMERS)
